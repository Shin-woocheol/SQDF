...
def aesthetic():
    ...
    #config.num_samples = [32, 64, 4096, 8192]  # 1k samples take 1h/gpu

    
    # If training w/o optimism
    # config.train.optimism = 'none'
    # config.train.SGLD = 0.0001 # This determines the basic noise level of SGLD noise, only used with non-optmism algorithm
...
def aesthetic_sqdf_cm_buffer():
    config = general()

    config.prompt_fn = "simple_animals"
    config.eval_prompt_fn = "eval_simple_animals"
    config.samping_prompt_fn = 'simple_animals'
    ...
    ##
    config.num_outer_eval_imgs = 32 
    config.train.batch_size_per_gpu_available = 1
    ...
    ###### Consistency Model ######
    config.use_consistency_model = True

    ###### SQDF_hyperparameters #######
    config.sqdf_alpha = 2.0
    config.sqdf_gamma = 0.9

    ###### Buffer #####
    config.train.on_policy_batch_size = 0.5
    config.train.off_policy_batch_size = 0.5
    config.num_stored_traj = 0
    config.buffer_size = 1024
    config.buf_size_per_gpu = config.buffer_size // config.train.num_gpus
    config.buffer_method = "random"
    config.replay_ratio = 1 # fixed
    config.buffer_dir = ""
    config.buffer_init_per_loop = False
